...
const Register = () => {
  ...
  const [role, setRole] = useState<UserRole>('stager')
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      setLoading(true)
      const { error, success } = await signUp(email, password, role, fullName)
      ...
      if (success) {
        ...
        navigate('/login', { state: { message: 'Account created successfully! Please check your email to verify your account.' } })
      }
    } catch (err) {
  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      // Note: Successful redirect is handled by the OAuth provider
    } catch (err) {
      setError('An unexpected error occurred')
      console.error(err)
    } finally {
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="mt-4">
            ...
            >
              <option value="stager">Event Stager</option>
